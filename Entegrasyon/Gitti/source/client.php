...
class ggClient{
	...
	public function __construct(){
		...
		if(strlen($ini_array['auth_user'])!=0) {$this->auth_user = $ini_array['auth_user'];} else {/*throw new Exception("auth_user is null");*/echo "auth_user is null"."</br>";};
        if(strlen($ini_array['auth_pass'])!=0) {$this->auth_pass = $ini_array['auth_pass'];} else {echo "auth_user is null"."</br>"; }
	 ...
	 * @return Web Servis mesajini object olarak dondurur	
	 */
	public function createApplication($developerId = null, $applicationName = null, $description = null, $accessType = null, $appType = null, $descDetail = null){
	 ...
	 */
	public function getCategories($startOffset = 0, $rowCount = 2, $withSpecs,$withDeepest ){
	...
	/**
	 * Kullanıcılar, bu metot sayesinde GittiGidiyor kategori bilgilerini arayabilir ve
     * ürün kataloglarının detaylı bilgisine ulaşabilir.
	 *
	 * Detayli Bilgi: https://dev.gittigidiyor.com/servisler/searchCatalog-CatalogService-soap-anonymous
	 *
	 * Category Service
	 * Get Category Info Method
	 *
	 * @param string	categoryCode: String Kategori kodu
     * @param string    title başlığı.
	 * @param string	$withSpecs
	 * @param int startOffSet  Hangi katalog indeksinden başlanılacağı.
     * param  int rowCount Kaç kayıt getirilecek ?
	 * @return object Selected category info
	 */
	public function searchCatalog($categoryCode='', $title='', $startOffset = 0, $rowCount = 2){
		//$withSpecs = $this->booleanConvert($withSpecs);
